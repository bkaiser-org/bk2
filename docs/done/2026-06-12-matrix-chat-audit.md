...
6. ~~**SEC-3/4** — AppCheck on all matrix callables, provisioning gate + no UID fallback in `getMatrixCredentials`, server-side logout on credential clear.~~ **Done 2026-06-12** (AppCheck + 7-day TTL were already in place; added the `requireMatrixLocalpart` provisioning gate across all minting CFs and a server-side `/logout` on deliberate logout; one-token-per-session via ARCH-1). **Remaining: `pnpm run deploy:functions`** to activate the gate. See SEC-3 / SEC-4 status notes. *(Medium)*

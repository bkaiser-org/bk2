...
**Deployed 2026-06-12** (`pnpm run deploy:functions`). Remaining verification:
...
*Deploy prerequisites:* `firebase deploy --only firestore:rules` (read lock) + `pnpm run deploy:functions` (publicApi). Optional: set `PUBLIC_API_ALLOWED_ORIGINS` / `PUBLIC_API_ALLOWED_TENANTS`; add a `_rateLimits.expiresAt` TTL policy.

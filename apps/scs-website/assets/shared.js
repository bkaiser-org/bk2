      ...
      <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="pt-6 border-t border-slate-200 dark:border-slate-800 flex flex-wrap justify-between items-center gap-3 text-xs text-slate-500">
          ...
          <div class="flex gap-4">
            <a href="impressum.html" class="hover:text-scs-green" data-i18n="footer.imprint">Impressum</a>
            <a href="datenschutz.html" class="hover:text-scs-green" data-i18n="footer.privacy">Datenschutz</a>
          </div>
        </div>
      </div>
    </footer>`;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  initReveal();
  // Merge Firestore content overrides in the background; re-apply lang when ready
  if (typeof SCS_API !== 'undefined') {
    SCS_API.content().then(data => {
      if (data) {
        if (data.de) Object.assign(i18n.de, data.de);
        if (data.en) Object.assign(i18n.en, data.en);
        applyLang(currentLang);
      }
    }).catch(() => {});
  }
